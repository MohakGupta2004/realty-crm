...
export async function assignCampaing(req: Request, res: Response) {
  ...
  console.log(user);
  //  const lead = await Lead.findOne({ _id: req.body.leadId, realtorId: user.id }).select("_id");
  ...
  if (lead.realtorId.toString() != realtorId) {
    return res.status(404).json({message: "Lead does not exists"})
  }
  ...
  const assignedStatus = await SMS_Service.assignCampaign(
    lead._id.toString(),
    0,
    campaignId,
  );
  ...
  res
    .status(200)
    .send({ message: "Campaign assigned successfully", assignedStatus });
}
...
export async function assignCampaings(req: Request, res: Response) {
  ...
  // Verify all leads belong to the userGMAIL_PUBSUB_TOPIC.
  ...
  const invalidLeadIds = leadIds.filter(
    (id: string) => !validLeadIds.includes(id),
  );
...
export async function smsWorker(req: Request, res: Response) {
  try {
    ...
    if (
      !enrollmentId ||
      !campaignIdAtTimeOfScheduling ||
      stepIndexAtTimeOfScheduling === undefined
    ) {
...
export async function smsDispatchWorker(req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Scheduler dispatch failed" });
  }
}
...
export async function getSmsStatus(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch SMS settings" });
  }
}
  ...
  try {
    ...
    if (typeof hasSMSCampaignEnabled !== "boolean") {
      return res
        ...
        .json({
          success: false,
          message: "hasSMSCampaignEnabled must be a boolean",
        });
    }

    if (hasSMSCampaignEnabled) {
      const hasPhoneRecord = await SMSNumber.exists({
      ...
      if (!hasPhoneRecord) {
        return res.status(400).json({
          success: false,
          message:
            "You must complete SMS onboarding and acquire a phone number before enabling SMS campaigns.",
        });
      }
    }
    ...
    return res
      ...
      .json({
        success: false,
        message: "Failed to toggle SMS campaign status",
      });
  }
}
...
export async function smsScheduler(req: Request, res: Response) {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Hand-off complete" });
  } catch (e) {
    console.error("[SMS Controller] smsScheduler error:", e);
    return res
      .status(500)
      .json({ success: false, message: "Failed to schedule SMS" });
  }
}
